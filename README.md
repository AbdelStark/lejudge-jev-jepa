...
2. **Jev's confidence gate held 21–62 % of candidates.** Jev's per-step probabilities often sit in the uncertain band [0.3, 0.7]; the pre-registered gate (τ = 0.5) therefore zeroed many penalties, whereas the keyword judge is binary and never abstains. Study 3 below tests the gate as a pre-registered follow-up.
