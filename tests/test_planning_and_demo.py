...
def test_demo_parse_constraints():
    ...
    cs = DemoBackend.parse_constraints("Never let the T touch the left or right edge.\nBe gentle: move the block slowly.\n" + "x" * 300 + "\nfourth line ignored")
    assert len(cs) == 3
...
def test_report_builds_from_synthetic_tables(tmp_path):
    ...
    for cond, pv in (("lewm", 0.6), ("jev", 0.2), ("oracle", 0.1), ("keyword", 0.4)):
        for cset in ("spatial", "implicit"):
            for ep in range(12):
                rows.append({"run_id": f"{cond}_{cset}", "condition": cond, "constraint_set": cset, "seed": 0, "episode": ep, "success": bool(rng.random() < 0.8), "violation": bool(rng.random() < pv), "violation_steps": 1, "plan_time_p50_s": 1.0, "plan_time_p95_s": 1.2, "judge_calls": 6, "tokens_in": 100, "abstention_rate": 0.0, "lam": 1.0, "tag": "", "response_model": "jev-1.13.0"})
        ...
        for item in range(20):
            for cid in ("centre_avoid", "gentle"):
                for variant in ("canonical", "p1", "n1"):
                    ...
                    jrows.append({"item_id": f"i{item}", "source": "executed", "description": "probe-words", "judge": judge, "constraint": cid, "family": "never", "variant": variant, "text": "t", "repeat": 0, "score": score, "p": "[]", "confidence": None, "label": label, "label_steps": "[]", "failed": False, "cache_hit": True, "latency_ms_per_q": 1.0, "tokens_in_per_q": 10.0, "tokens_out_per_q": 1.0, "response_model": "x", "bank": "b", "library": "l", "vocab": "v"})
