...
def bucket_accuracy(pred: SymbolicState, truth: SymbolicState, vocab: Vocab) -> dict[str, float]:
    ...
    out["block_edge"] = float(
        np.mean(
            vocab.edge_name(pred.block_xy, pred.block_angle)
            == vocab.edge_name(truth.block_xy, truth.block_angle)
        )
    )
    out["block_angle"] = float(
        np.mean(vocab.angle_name(pred.block_angle) == vocab.angle_name(truth.block_angle))
    )
...
def evaluate_probe(
    probe: torch.nn.Module, data: EpisodeData, eps: np.ndarray, vocab: Vocab
) -> dict[str, Any]:
    m = data.mask(eps)
    x = data.emb[m]
    ...
    metrics: dict[str, Any] = {
        "n": int(m.sum()),
        "r2": {
            "agent_xy": r2(y[:, 0:2], pred[:, 0:2]),
            "block_xy": r2(y[:, 2:4], pred[:, 2:4]),
            "angle_sincos": r2(y[:, 4:6], pred[:, 4:6]),
        },
        "mse": {
            "agent_xy": float(np.mean((y[:, 0:2] - pred[:, 0:2]) ** 2)),
            "block_xy": float(np.mean((y[:, 2:4] - pred[:, 2:4]) ** 2)),
            "angle_sincos": float(np.mean((y[:, 4:6] - pred[:, 4:6]) ** 2)),
        },
        "position_error_px": {
            "agent": float(np.mean(np.linalg.norm(y[:, 0:2] - pred[:, 0:2], axis=1)) * 512),
            "block": float(np.mean(np.linalg.norm(y[:, 2:4] - pred[:, 2:4], axis=1)) * 512),
        },
        "contact": {
            "auroc": auroc(y[:, 6], pred[:, 6]),
            "accuracy": float(np.mean((pred[:, 6] > 0) == (y[:, 6] > 0.5))),
        },
    }
...
def imagined_curve(
    ...
    action_seq = torch.tensor(np.stack(acts), dtype=torch.float32, device=device).unsqueeze(
        0
    )  # (1, N, H, 10)
        ...
        curve.append(
            {
                "h": h,
                "bucket_accuracy": acc,
                "block_error_px": pos_err,
                "agent_error_px": agent_err,
                "angle_error_deg": float(np.degrees(d.mean())),
            }
        )
    return {
        "n_starts": len(starts),
...
def train(
    ...
    ytr = targets_from_state(
        data.state[data.mask(split["train"])], data.contact[data.mask(split["train"])]
    )
    yva = targets_from_state(
        data.state[data.mask(split["val"])], data.contact[data.mask(split["val"])]
    )
    probe: LinearProbe | MLPProbe = LinearProbe() if kind == "linear" else MLPProbe()
    hp = (
        probe.fit(xtr, ytr, x_val=xva, y_val=yva)
        if kind == "linear"
        else probe.fit(xtr, ytr, xva, yva, seed=seed)
    )
    probe.eval()
...
def summary(meta_path: Path | str) -> str:
    ...
    lines.append(
        "bucket acc: " + ", ".join(f"{k}={v:.3f}" for k, v in t["bucket_accuracy"].items())
    )
    lines.append(
        f"contact AUROC={t['contact']['auroc']:.3f} acc={t['contact']['accuracy']:.3f}; angle err={t['angle_error_deg']:.1f} deg; block err={t['position_error_px']['block']:.1f}px"
    )
    if "imagined" in m:
        for row in m["imagined"]["curve"]:
            ba = row["bucket_accuracy"]
            lines.append(
                f"  h={row['h']}: block={ba['block']:.2f} edge={ba['block_edge']:.2f} angle={ba['block_angle']:.2f} agent={ba['agent']:.2f} contact={ba['contact']:.2f} | block err {row['block_error_px']:.1f}px"
            )
    return "\n".join(lines)
