...
def uncertainty_proxy(family: str, p: list[float], confidence: float | None, band: tuple[float, float] = (0.3, 0.7)) -> float:
    """Fraction of step probabilities inside the uncertain band (never/always) or 1 − confidence (soft)."""
    if family == "soft":
        return 1.0 - float(confidence) if confidence is not None and not math.isnan(confidence) else 1.0
    if not p:
        return 1.0
    arr = np.asarray(p, dtype=np.float64)
